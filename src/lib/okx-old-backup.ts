...
export async function placeOrder(
  ...
  try {
    ...
    // 🔧 CRITICAL FIX: 统一使用CONTRACT_VALUES
    // 逻辑：
    // - 输入amount = 想下多少张合约（如3张BTC）
    // - 需要的币数量 = amount × contractValue（3张 × 0.01 = 0.03 BTC）
    // - ccxt接受币数量，所以直接传币数量
    const coinsAmount = amount * contractValue;
    const ccxtAmount = coinsAmount;  // ccxt使用币数量
    
    // ⚠️ 检查最小精度
    if (ccxtAmount < 0.0001) {
      const errorMsg = `合约数量不足最小精度要求 (${amount.toFixed(8)}张 = ${ccxtAmount.toFixed(8)}币 < 0.0001)。建议：${
        reduceOnly 
    ...
    console.log(`[placeOrder] 币数量: ${amount.toFixed(8)} × ${contractValue} = ${coinsAmount.toFixed(8)} ${coin}`);
    console.log(`[placeOrder] CCXT数量: ${ccxtAmount.toFixed(8)} ${coin}`);
